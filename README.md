...
| `topic`   | id темы из `content/topics.json` |
...
- Каждая непустая листовая тема должна содержать ≥4 структур.
...
`sideFor(id, en)` в `scripts/content-rules.ts` сначала смотрит в
