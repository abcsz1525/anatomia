...
- Знак ударения — комбинирующий акут U+0301 после ударной гласной (`а́`). На односложных словах и на словах из одной гласной ударение не ставится.
...
- `l` — **мягкое**: `la` → ля, `lo` → лё, `lu` → лю, `le` → ле, `li` → ли, `ly` → ли; перед согласной и в конце слова → ль. `ll` → лл по тем же правилам (`maxilla` → макси́лля).
